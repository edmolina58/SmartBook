...
public class IngresoRepository : IIngresoRepository
{
    ...
    public void Crear(Ingreso ingreso)
    {
        ...
        using (var conexion = new MySqlConnection(_connectionStrings))
        {
            ...
            Sql = @"
                ...
                UPDATE `smartbook`.`libros` SET `stock` = @unidades WHERE (`id_libro` = @id); ";
    ...
    public IEnumerable<ConsultarIngresosResponse> ConsultarPorFecha(DateOnly fechainicio, DateOnly fechaFin)
    {
        ...
        using (var conexion = new MySqlConnection(_connectionStrings))
        {
            ...
            Sql = "SELECT idIngreso,libro,Unidades,valor_compra,valor_venta,fecha FROM ingresos WHERE fecha BETWEEN @fechainicio AND @fechaFin";
            ...
            using (var cmd = new MySqlCommand(Sql, conexion))
            {
                ...
                while (reader.Read())
                {
                    ...
                    var valorCompra = Convert.ToDouble(reader["valor_compra"]);
                    var valorVenta = Convert.ToDouble(reader["valor_venta"]);
